...
- `UNIDENTIFIABLE_POINT_ESTIMATE_CLAIM`：identifiability diagnostic 已判定参数不可识别，但结果仍登记非空点估计及其 metric，并由 final 定量 claim 引用；G4 阻断并令该 result 失去 eligibility。旧 `2.4.x` 使用对应的 `UNIDENTIFIABLE_POINT_ESTIMATE_CLAIM_MIGRATION_REQUIRED`，不在 Schema 层崩溃。
