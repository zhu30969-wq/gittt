...
# current generators move to 2.5.0.  It proves that a valid 2.0.x predecessor
...
class InitializationAndRunTimeTests(unittest.TestCase):
    ...
    def test_current_2_5_0_experiment_still_requires_decision_timing_in_schema(self) -> None:
        """The legacy exception must not weaken newly generated 2.5 contracts."""
        ...
        self.assertEqual(0, first_code, first_report)
        experiment = load_yaml(target / "experiments" / "experiment.yaml")
        self.assertEqual("2.5.0", experiment["schema_version"])
