...
- `assets/predict-toolkit` 把嵌套样本外验证、时序可用性、校准、漂移和排名翻转整理为可复用接口；
- `assets/mechanism-toolkit` 提供 ODE、方法线一维传热、多起点参数辨识和蒙特卡洛/准蒙特卡洛；缺少合格收敛证书或参数不可识别时结构性清空过强点值；
...
├── test_executable_scenarios.py
├── test_forward_testing_doc.py
├── test_identifiability_guard.py
...
├── e21_identifiability_guard_scenario.json
└── run_identifiability_guard.py
~~~
